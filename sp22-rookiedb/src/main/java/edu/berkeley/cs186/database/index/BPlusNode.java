...
abstract class BPlusNode {
     ...
     * leaf1.getLeftmostLeaf() would return leaf1. c
     */
     ...
     *           Optional.empty() is returned.就是 keys.size() <= 2*order
     ...
     *          返回 pair(split_key, right_node_page_num)
     ...
     * moved  up as the split key(not copied). For example, we would split the
     ...
     * splitting.
     *
     * Our B+ trees do not support duplicate entries with the s0ame key. If a
    ...
    public static BPlusNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Page p = bufferManager.fetchPage(treeContext, pageNum);// 获取page
        try {
            Buffer buf = p.getBuffer();
            byte b = buf.get();//节点的标志位
            if (b == 1) {

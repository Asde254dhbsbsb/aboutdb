...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        // 从root里开始递归，在内节点里面寻找,但是需不需要判断root是不是内节点呢？
        //root.get(key).getKey(key);在这个节点里寻找keys
//        System.out.println("page:"+root.getPage().getPageNum());
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        // TODO(proj2): Return a BPlusTreeIterator.
        LeafNode leafNode = root.get(key);// 获取
        int index = 0;

        while(key.compareTo(leafNode.getKeys().get(index)) > 0) {
            index++;
        }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if(op.isPresent()) {
            ...
            BPlusNode newBPlusNode = new InnerNode(metadata, bufferManager, keys, children, lockContext);
            updateRoot(newBPlusNode);
        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private LeafNode tmpNode;
        private int index;

        public BPlusTreeIterator(LeafNode bPlusNode) {
            super();
            this.tmpNode = bPlusNode;
        }

        public BPlusTreeIterator(LeafNode bPlusNode, int index) {
            super();
            this.index = index;
            this.tmpNode = bPlusNode;
        }
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            Optional<LeafNode> rightNode = tmpNode.getRightSibling();
            if(index == tmpNode.getRids().size()) {
                return rightNode.isPresent() && rightNode.get().getRids().size() != 0;
            }
            return true;
        ...
        public RecordId next() {
            ...
            if(hasNext()) {
                if(index == tmpNode.getRids().size()) {
                    tmpNode = tmpNode.getRightSibling().get();
                    index = 0;
                    return tmpNode.getRids().get(index++);
                }
                return tmpNode.getRids().get(index++);
            }

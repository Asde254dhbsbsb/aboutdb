...
class InnerNode extends BPlusNode {
    ...
    // 内节点（键，值==>指向孩子页的指针）和叶子节点（键， 值==>真实记录值）的区别
    ...
    public LeafNode get(DataBox key) {
        ...
        // 从一系列的keys寻找，找到children的pageId
        int i = 0;
        while(i < keys.size() && keys.get(i).compareTo(key) <= 0)
            i++;

        BPlusNode bn = BPlusNode.fromBytes(metadata, bufferManager, treeContext, children.get(i));
        return bn.get(key);
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
//        需要递归调用先获取下一级的返回 可能是空也可能是需要到插入内节点的键值对
        int i = 0;
        while(i < keys.size() && keys.get(i).compareTo(key) <= 0)
            i++;
        Optional<Pair<DataBox, Long>> op = BPlusNode.fromBytes(metadata, bufferManager, treeContext, children.get(i)).put(key, rid);
         // 根据op是否为空来返回
        if(op.isPresent()) {

            // 在该节点插入key : DataBox 以及新增儿子；
            int index = 0;
            // 在keys里面寻找index；
            while(index < keys.size() && op.get().getFirst().compareTo(keys.get(index)) > 0) index++;

            keys.add(index, op.get().getFirst());
            children.add(index + 1, op.get().getSecond());

            // 如果发送溢出的话，就进行分裂  前d个不变 后d个新增 中间一个上浮
            if(keys.size() > 2 * metadata.getOrder()) {
                 List<DataBox> keys2 = keys.subList(metadata.getOrder() + 1, 2 * metadata.getOrder() + 1);
                 List<Long> children2 = children.subList(metadata.getOrder() + 1, 2 * metadata.getOrder() + 2);
                 // 新增内节点 不需要右兄弟
                InnerNode innerNode = new InnerNode(metadata, bufferManager, keys2, children2, treeContext);
                DataBox sp = keys.get(metadata.getOrder());// 需要先获取中间节点再截取
                Long spPageNum = innerNode.getPage().getPageNum();
                keys = keys.subList(0, metadata.getOrder() );
                children = children.subList(0, metadata.getOrder() + 1);

                sync();
                return Optional.of(new Pair<>(sp, spPageNum));
            }
        }
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // 当仍然有data而且
        while(data.hasNext()) {
            Optional<Pair<DataBox, Long>> pdr = BPlusNode.fromBytes(metadata, bufferManager, treeContext, getChildren().get(getChildren().size() - 1)).bulkLoad(data, fillFactor);
            if(pdr.isPresent()){// 如果返回有就插入
                // ＋1超了的话
                if (keys.size() + 1 > 2 * metadata.getOrder() * fillFactor) {
                    // 再加一个就返回的话 此时直接分裂然后返回
                    List<DataBox> keys2 = new ArrayList<>();
                    List<Long> children2 = new ArrayList<>();
                    keys2.add(pdr.get().getFirst());
                    children2.add(pdr.get().getSecond());
                    InnerNode innerNode = new InnerNode(metadata, bufferManager, keys2, children2, treeContext);

                    DataBox dataBox = keys2.get(0);
                    Long pageNum = innerNode.page.getPageNum();
                    sync();
                    return Optional.of(new Pair<>(dataBox, pageNum));
                } else {// 没有满的话！
                    keys.add(pdr.get().getFirst());
                    children.add(pdr.get().getSecond());
                    sync();
                }
            }
    ...
    public static InnerNode fromBytes(BPlusTreeMetadata metadata,
        ...
        Buffer buf = page.getBuffer();  // 获取缓冲区

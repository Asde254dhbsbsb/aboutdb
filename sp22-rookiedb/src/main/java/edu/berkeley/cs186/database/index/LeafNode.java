...
class LeafNode extends BPlusNode {
    ...
    // The keys and record ids of this leaf. `keys` is always sorted in ascending升序
    ...
    // [a:1, b:2, c:3].
    //
    // Note the following subtlety. keys and rids are in-memory caches of the keys and record ids stored on disk.
    // Thus, consider what happens when you
     ...
     * to that page.
     * 构造一个全新的叶节点
     * 为bufferManager获取一个新的固定页面并将节点持久化到page
     */
    ...
    public LeafNode get(DataBox key) {
        ...
        return fromBytes(metadata, bufferManager, treeContext, page.getPageNum());
    }
    ...
    public static int binSearch(List<DataBox> list, DataBox key) {
        if(list.isEmpty()) return 0;

        int left = 0;
        int right = list.size() - 1;

        if(list.get(left).compareTo(key) == 0) return -1;
        else if(list.get(left).compareTo(key) > 0) return 0;
        else if(right - left == 0) return 1;

        if(list.get(right).compareTo(key) < 0) return right + 1;

        while (left < right) {
            int middle = left + (right - left) / 2;
            if(list.get(middle).compareTo(key) == 0) return -1;
            else if(list.get(middle).compareTo(key) < 0) left = middle + 1;
            else right = middle;
        }
        return left;
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        int index = binSearch(keys, key);
        if(index == -1) {
            throw new BPlusTreeException("error:the same key!");
        }
        keys.add(index, key);
        rids.add(index, rid);
        if(keys.size() > 2*metadata.getOrder()) {
            List<DataBox> keys2 = keys.subList(metadata.getOrder(), 2 * metadata.getOrder() + 1);
            List<RecordId> rids2 = rids.subList(metadata.getOrder(), 2 * metadata.getOrder() + 1);
            LeafNode leafNode2 = new LeafNode(metadata, bufferManager, keys2, rids2, rightSibling, treeContext);
            rightSibling = Optional.of(leafNode2.getPage().getPageNum());
            keys = keys.subList(0, metadata.getOrder());
            rids =  rids.subList(0, metadata.getOrder());
            sync();
            return Optional.of(new Pair<>(keys2.get(0), leafNode2.getPage().getPageNum()));
        }
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // 一直往右边添加直到满了然后分裂再返回
        while(data.hasNext()){
            Pair<DataBox, RecordId> pdr =  data.next();
            if(keys.size() + 1 > 2 * metadata.getOrder() * fillFactor) {
                // 此时直接分裂
                List<DataBox> keys2 = new ArrayList<>();
                List<RecordId> rids2 = new ArrayList<>();
                keys2.add(pdr.getFirst());
                rids2.add(pdr.getSecond());
                LeafNode leafNode2 = new LeafNode(metadata, bufferManager, keys2, rids2, Optional.empty(), treeContext);
                rightSibling = Optional.of(leafNode2.getPage().getPageNum());
                sync();
                return Optional.of(new Pair<>(pdr.getFirst(), leafNode2.getPage().getPageNum()));
            } else {
                keys.add(pdr.getFirst());
                rids.add(pdr.getSecond());
                sync();
            }
    ...
    public void remove(DataBox key) {
        ...
        // 找到key并remove
        int i = 0;
        while(i < keys.size() && !keys.get(i).equals(key)) i++;
        if(i == keys.size()) throw new BPlusTreeException("没有这样的键");
        keys.remove(i);
        rids.remove(i);
        sync();
        return;
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        if(rs == (long) -1) rightSibling = Optional.empty();
        else rightSibling = Optional.of(rs);

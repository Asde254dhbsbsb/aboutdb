 ...
 * 数据存储在paged-size的字节数组，并且从加载页面特定的Frame对象中返回
 * 退出并加载一个新的页面将会
 * 生成一个新的具有相同的底层字节数组的Frame对象，而旧的会被标记为无效
 */
public class BufferManager implements AutoCloseable {
    ...
    // fit on one page).
//    每页预留36个字节用来簿记以便恢复（用来储存pageLSN以及确保页面可以容纳进一个重做或回滚日志）

...
QueryPlan {
    ...
    // A list of objects representing joins (INNER JOIN clauses)表示连接的对象列表
    ...
    public String toString() {
        ...
        // FROM clause
        String baseTable = this.tableNames.get(0);// 本名
        String alias = aliases.get(baseTable);  // as 别名
        if (baseTable.equals(aliases.get(baseTable)))
            result.append(String.format("\nFROM %s\n", baseTable));
        else result.append(String.format("\nFROM %s AS %s\n", baseTable, alias));
        // INNER JOIN clauses   内连接语句
...
//      是否用别名
    ...
    /**
     * 将最终运算符设置为投影运算符，并将最开始的最终运算符作为其源。如果没有投影列，则什么也不做。
    ...
    /**
     * 如果指定了排序，而 final operator 还没有排序，则将最终操作符设置为排序操作符。
    ...
    /**
     * Add a select operator. Only returns columns in which the column fulfills the predicate relative to value.
     *
     * 添加一个选择运算符。只返回列（其上有满足判断的值）
     *
...
//            有索引，而且条件不是 NOT_EQUALS
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int indexMinIO = minOp.estimateIOCost();

        for (Integer eligibleIndexColumn : eligibleIndexColumns) {
            SelectPredicate p = this.selectPredicates.get(eligibleIndexColumn);
            QueryOperator indexOp = new IndexScanOperator(this.transaction, table, p.column, p.operator, p.value);
            if (indexOp.estimateIOCost() < indexMinIO) {
                indexMinIO = indexOp.estimateIOCost();
                minOp = indexOp;
                minIndex = i;
            }
            i++;
     ...
     *
     * 给定左操作符和右操作符之间的连接谓词，
     * 在JoinOperator.JoinType中从连接类型中查找开销最低的连接操作符。
     * 默认情况下，只考虑SNLJ和BNLJ以防止对GHJ、Sort和SMJ的依赖。
     *
     ...
     *
     *在前一次搜索中遍历所有表集。 对于每个表集，检查每个连接谓词，看看是否存在与新表的有效连接。
     * 如果是，找到最小开销连接。 返回连接到其最低成本连接操作符的每一组表名的映射。
     *
     ...
     * The number of elements in each set of table names should be equal to the pass number.
     */
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        // For each set of tables in prevMap    逻辑实现
        ...
        for (Set<String> preTableSet: prevMap.keySet()) {
            for(JoinPredicate joinPredicate : this.joinPredicates) {
                Set<String> newSet = new HashSet<>(preTableSet); //由于使用foreach所以preTableSet是临时变量
                Set<String> tableSet = new HashSet<>();          // pass1的单表集合
                QueryOperator joinQuery, leftOperator, rightOperator;

                if(preTableSet.contains(joinPredicate.leftTable) && !preTableSet.contains(joinPredicate.rightTable)) {
                    tableSet.add(joinPredicate.rightTable); rightOperator = pass1Map.get(tableSet); //单表所以只有一个！

                    leftOperator = prevMap.get(preTableSet);
                    joinQuery = minCostJoinType(leftOperator, rightOperator, joinPredicate.leftColumn, joinPredicate.rightColumn);
                    newSet.add(joinPredicate.rightTable);
                } else if(preTableSet.contains(joinPredicate.rightTable) && !preTableSet.contains(joinPredicate.rightTable)) {
                    tableSet.add(joinPredicate.leftTable);leftOperator = pass1Map.get(tableSet);

                    rightOperator = prevMap.get(preTableSet);
                    joinQuery = minCostJoinType(leftOperator, rightOperator, joinPredicate.leftColumn, joinPredicate.rightColumn);
                ...
                if (result.containsKey(newSet)) { // 存在这个newSet的话，joinQuery必须更小才可能更新result
...
//        pass --- 1
        for(String table : this.tableNames) {
            ...
            pass1Map.put(new HashSet<>(singleTableSet), minCostSingleAccess(table));
        }
//        pass --- 2->n

...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        while(records.hasNext()) runList.add(records.next());
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        PriorityQueue<Pair<Record, Integer>> runPQ = new PriorityQueue<>(new RecordPairComparator());
        Run mergerSortedRun = makeRun();
//        为什么迭代器next不变呢？
        ...
        for(int i = 0; i < runs.size(); i++)
            if(iters.get(i).hasNext()) {
                runPQ.add(new Pair<>(iters.get(i).next(), i));
            }

        while(!runPQ.isEmpty()) {
            Pair<Record, Integer> recordI =  runPQ.poll();
            mergerSortedRun.add(recordI.getFirst());
            int i = recordI.getSecond(); //第几个runList
            if(iters.get(i).hasNext())
                runPQ.add(new Pair<>(iters.get(i).next(), i));
        }

        return mergerSortedRun;
    }
     ...
     *
     * N个， 你一次只能合并numBuffers - 1个
     * 一次运行合并numBuffers - 1的结果   N
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        List<Run> runs2 = new ArrayList<>();// 直到runs2的size小于等于numBuffers - 1就可以返回了
        for (int i = 0; i + workSize <= N ; i += workSize) {
            runs2.add(mergeSortedRuns(runs.subList(i, Math.min(N, i + workSize))));
        }
        return runs2;
    }

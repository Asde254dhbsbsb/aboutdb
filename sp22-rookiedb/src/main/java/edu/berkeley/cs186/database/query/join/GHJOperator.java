...
//            执行GHJ on-the-fly是很困难的在没有协同程序的情况下，所有代替的是在整个运行中累计所有连接记录
     ...
     * partitions.
     *  对于给定迭代器中的每一条记录，将我们要连接的列的值散列，并将其添加到分区中的正确分区。
...
//        Build
        for(Record buildRecord : buildRecords) {
            DataBox buildColumnValue = buildRecord.getValue(buildColumnIndex);

            if(!hashTable.containsKey(buildColumnValue)) {
                hashTable.put(buildColumnValue, new ArrayList<>());
            }

            hashTable.get(buildColumnValue).add(buildRecord);
        }

//        Probe
        for(Record probeRecord : probeRecords) {
            DataBox probeColumnValue = probeRecord.getValue(probeColumnIndex);

            if(!hashTable.containsKey(probeColumnValue)) continue;

            for(Record buildRecord : hashTable.get(probeColumnValue)) {
                Record result = probeFirst ? probeRecord.concat(buildRecord) : buildRecord.concat(probeRecord);
                joinedRecords.add(result);
            }
        }
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (Partition leftPartition : leftPartitions) { // 已经分了这么多个区
            ...
            for (Partition rightPartition : rightPartitions) {
                try {
                    this.buildAndProbe(leftPartition, rightPartition);
                } catch (Exception IllegalArgumentException) {
                    // 说明两个分区都很大,接着对left和right分区
...
//        2--3--4---5
        for(int i = 0; i < 160; i++) {
            ...
            leftRecords.add(createRecord(5));

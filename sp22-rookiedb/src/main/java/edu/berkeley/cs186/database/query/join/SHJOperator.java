...
public class SHJOperator extends JoinOperator {
     ...
     *
     * 对于左记录迭代器中的每个记录，散列该值并将记录添加到正确的分区
     */
    ...
    /**
     ...
     joinedRecords list.
     *
     * 使用leftRecords构建哈希表，并使用rightRecords中的记录探测它。
     * 连接匹配的记录，并将它们作为joinedRecords列表返回。
     *
...
//        我们构建的哈希表，表里包含左记录中散列到相同键的所有记录
        Map<DataBox, List<Record>> hashTable = new HashMap<>();
        /** 在各个分区里面进行散列*/
        // Building stage
        for (Record leftRecord: partition) { /** 分区迭代器 */
        ...
        for (Record rightRecord: rightRecords) {
            ...
            if (!hashTable.containsKey(rightJoinValue)) continue; /**哈希表没有匹配说明这个右记录在这个分区没有匹配的左值接着循环*/

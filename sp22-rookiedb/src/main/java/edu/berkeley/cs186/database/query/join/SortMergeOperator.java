    ...
    private class SortMergeIterator implements Iterator<Record> {
        ...
        */
        ...
        private Record fetchNextRecord() {
            ...
            while(true) {
                if(!this.marked) {
                    while(compare(leftRecord, rightRecord) < 0) // leftRecord < rightRecord意味着leftRecord需要++
                        if (leftIterator.hasNext()) leftRecord = leftIterator.next();
                        else return null;
                    while(compare(leftRecord, rightRecord) > 0)// leftRecord > rightRecord意味着rightRecord需要++
                        if(rightIterator.hasNext()) rightRecord = rightIterator.next();
                        else return null;
                    this.marked = true;
                    rightIterator.markPrev(); //对右边做标记
                }
                if(compare(leftRecord, rightRecord) == 0) {// 相等的话
